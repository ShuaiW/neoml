# -*- coding: utf-8 -*-
"""
Created on Sat Jan 30 18:27:19 2016
...
class TwoLayerNet(object):
    ...
    regularization on the weight matrices. The network uses a ReLU 
    ...
    def __init__(self, input_size, hidden_size, output_size, std=1e-4):
        ...
        # another way to initialize the weights    
...
#        self.params['W2'] = np.random.randn(hidden_size, output_size) / np.sqrt(2.0/hidden_size)
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        ### Forward pass (staged version)
        dot1 = X.dot(W1)                    # (N, H)    1
        add_b1 = dot1 + b1                  # (N, H)    2
        relu = RELU(add_b1)                 # (N, H)    3
        dot2 = relu.dot(W2)                 # (N, C)    4
        scores = dot2 + b2                  # (N, C)    5
        ...
        logloss = -scores[np.arange(N), y] + np.log(np.sum(exp_scores, axis=1))
        ...
        dscores /= N        # total error 1; distributed acorss all data points
        
        ddot2 = dscores                     # (N, C)    5
        db2 = np.sum(dscores, axis=0)       # (C, )     5
        dW2 = relu.T.dot(ddot2)             # (H, C)    4
        drelu = ddot2.dot(W2.T)             # (N, H)    4
        dadd_b1 = (add_b1 > 0) * drelu      # (N, H)    3
        ddot1 = dadd_b1                     # (N, H)    2
        db1 = np.sum(dadd_b1, axis=0)       # (H, )     2
        dW1 = X.T.dot(ddot1)                # (D, H)    1
